...
def dipole_magnetic(coordinates, dipoles, magnetic_moments):
    ...
    # Herda o tipo dos dados de entrada (se entrar float32, sai float32)
    dtype = coordinates[0].dtype 
    magnetic_field = [np.zeros(coordinates[0].shape, dtype=dtype) for i in range(3)]
...
class EquivalentSourcesMagnetic():
    ...
    def __init__(
        ...
        dipole_inclination=90, dipole_declination=0, dipole_coordinates=None, dtype="float64",
    ):
    ...
    def fit(self, coordinates, data, field_direction, weights=None, dtype=None):
        """
        """
        
        # --- Lógica de seleção do dtype --- LINHA ALTERADA
        if dtype is None:
            dtype_to_use = self.dtype
        else:
            dtype_to_use = dtype
        ...
        # --- Conversão da precisão --- 
        coordinates, data, weights = cast_fit_input(
            coordinates, data, weights, dtype_to_use    # <-- Usa o dtype escolhido LINHA ALTERADA
        )
        ...
        jacobian = self.jacobian(
            coordinates, self.dipole_coordinates_, dipole_moment_direction, field_direction, dtype=dtype_to_use  # <-- Usa o dtype escolhido LINHA ALTERADA
        )
    ...
    def jacobian(
        self, coordinates, dipole_coordinates, dipole_moment_direction, field_direction, dtype="float64"
    ):
        """
        """
        n = len(coordinates[0])
...
class EquivalentSourcesMagneticGB(EquivalentSourcesMagnetic):
    ...
    def __init__(
        ...
        window_size=None, repeat=1, random_state=None, dtype="float64",
    ):
        super().__init__(
            damping, depth, block_size, dipole_inclination, dipole_declination,
            dipole_coordinates, dtype=dtype
        )
    ...
    def fit(self, coordinates, data, field_direction, weights=None, dtype=None):
        ...
        # --- Conversão da precisão --- LINHA ALTERADA
        coordinates, data, weights = cast_fit_input(
            coordinates, data, weights, dtype_to_use 
        )
        ...
        residuals = data.copy()
        moment_amplitude = np.zeros_like(self.dipole_coordinates_[0], dtype=dtype_to_use)   # <-- Usa o dtype escolhido LINHA ALTERADA
        ...
        for iteration in range(self.repeat):
            ...
            for window in window_indices:
                ...
                if weights is not None:
                    ...
                    weights_chunk = None
                jacobian = self.jacobian(
                    coords_chunk, dipole_chunk, dipole_moment_direction, field_direction, dtype=dtype_to_use  # <-- Usa o dtype escolhido LINHA ALTERADA
                )
