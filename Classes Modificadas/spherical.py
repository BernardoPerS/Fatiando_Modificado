...
def dipole_magnetic_spherical(coordinates, dipoles, magnetic_moments):
    ...
    ### -------- LINHA ALTERADA --------- ### Mudando dtype nos arrays de saída
    output_dtype = coordinates[0].dtype 

    # Initialize the output arrays
    b_lon = np.zeros(n_data, dtype=output_dtype)
    b_lat = np.zeros(n_data, dtype=output_dtype)
    b_radial = np.zeros(n_data, dtype=output_dtype)
...
class EquivalentSourcesMagGeod:
    ...
    def fit(self, coordinates, inclination, declination, data, weights=None, dtype=None):  ### ----- LINHA ALTERADA ----- ### adição do parâmetro dtype 


        ### ---------- LINHA ALTERADA ------------- ###
        # Altera todos os dados para o data type passado por parâmetro
        if dtype is not None:
            coordinates = tuple(np.asarray(c, dtype=dtype) for c in coordinates)
        ...
        unit_moment = vector_geodetic_to_spherical(
            *hm.magnetic_angles_to_vec(
                np.ones(n_params, dtype=data.dtype), self.inclination, self.declination  ### -------- LINHA ALTERADA --------- ### Adicionando dtype na criação dos vetores
            ),
            latitude_spherical=source_coordinates_sph[1],
            latitude=self.source_coordinates_[1],
        )
        main_field = vector_geodetic_to_spherical(
            *hm.magnetic_angles_to_vec(np.ones(n_data, dtype=data.dtype), inclination, declination),  ### -------- LINHA ALTERADA --------- ### Adicionando dtype na criação dos vetores
            latitude_spherical=coordinates_sph[1],
            latitude=coordinates[1],
        )
        # Solve the inverse problem
        jacobian = self._jacobian(
            coordinates_sph, source_coordinates_sph, unit_moment, main_field, dtype=data.dtype
        )
    ...
    def _jacobian(
        self, coordinates_sph, source_coordinates_sph, unit_moment, main_field, dtype="float64"  ### ----- LINHA ALTERADA ----- ### adição do parâmetro dtype 
    ):
        """ """
        n_data = coordinates_sph[0].size
        n_params = source_coordinates_sph[0].size
        jacobian = np.empty((n_data, n_params), dtype=dtype)
        _jacobian_fast(
...
class EquivalentSourcesMagGeodGB(EquivalentSourcesMagGeod):
    ...
    def fit(self, coordinates, inclination, declination, data, weights=None, dtype=None):  ### ----- LINHA ALTERADA ----- ### adição do parâmetro dtype 
        ...
        moment_amplitude = np.zeros(n_params, dtype=data.dtype) ### -------- LINHA ALTERADA --------- ### Adicionando dtype na inicialização da solução
        ...
        for i in window_indices:
            ...
            jacobian = self._jacobian(
                ...
                unit_moment_window,
                main_field_window,
                dtype=data.dtype,  ### -------- LINHA ALTERADA --------- ### Adicionando dtype à chamada da jacobiana
            )
