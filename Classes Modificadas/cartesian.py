...
class EquivalentSources(vdb.BaseGridder):
    ...
    * Analytical derivative calculations
    ...
    def fit(self, coordinates, data, weights=None, dtype=None):
        """
        ...
        coordinates, data, weights = cast_fit_input(
            coordinates, data, weights, dtype_to_use   # <-- Usa o dtype escolhido
        )
        ...
        if self.points is None:
            self.points_ = tuple(
                p.astype(dtype_to_use) for p in self._build_points(coordinates)   # <-- Usa o dtype escolhido
            )
        else:
            self.depth_ = None  # set depth_ to None so we don't leave it unset
            self.points_ = tuple(
                p.astype(dtype_to_use) for p in vdb.n_1d_arrays(self.points, 3)   # <-- Usa o dtype escolhido
            )
        jacobian = self.jacobian(coordinates, self.points_, dtype=dtype_to_use)   # <-- Usa o dtype escolhido
